...
AABCharacterPlayer::AABCharacterPlayer()
{
	...
	static ConstructorHelpers::FObjectFinder<UInputMappingContext> MouseLookMappingContextRef(TEXT("/Script/EnhancedInput.InputMappingContext'/Game/ArenaBattle/Input/IMC_MouseLook.IMC_MouseLook'"));
	if (nullptr != MouseLookMappingContextRef.Object)
	{
		MouseLookMappingContext = MouseLookMappingContextRef.Object;
	}
	...
	static ConstructorHelpers::FObjectFinder<UInputAction> InputActionMouseLookRef(TEXT("/Script/EnhancedInput.InputAction'/Game/ArenaBattle/Input/Actions/IA_MouseLook.IA_MouseLook'"));
...
void AABCharacterPlayer::BeginPlay()
{
	...
	if (UEnhancedInputLocalPlayerSubsystem* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(PlayerController->GetLocalPlayer()))
	{
		...
		Subsystem->AddMappingContext(MouseLookMappingContext, 1); // IMC_MouseLook 추가 (우선순위는 Default보다 낮게)
...
void AABCharacterPlayer::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// 기존 LookAction 바인딩 (게임패드용 등으로 남겨두려면 유지)
	// EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Triggered, this, &AABCharacterPlayer::Look);

	// MouseLookAction 바인딩 추가 (마우스용)
	EnhancedInputComponent->BindAction(MouseLookAction, ETriggerEvent::Triggered, this, &AABCharacterPlayer::Look); // MouseLookAction도 Look 함수 호출
}
